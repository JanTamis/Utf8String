...
internal static partial class Marvin
{
	...
	private static int ComputeHash32OrdinalIgnoreCaseSlow(ref char data, int count, uint p0, uint p1)
	{
		...
		var scratch = (uint)count <= 64 ? stackalloc char[64] : borrowedArr = ArrayPool<char>.Shared.Rent(count);

		var charsWritten = MemoryMarshal.CreateReadOnlySpan(ref data, count).ToUpperInvariant(scratch);

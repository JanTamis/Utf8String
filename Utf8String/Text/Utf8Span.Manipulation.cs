...
public readonly ref partial struct Utf8Span
{
	public SplitResult Split(char separator, Utf8StringSplitOptions options = Utf8StringSplitOptions.None)
	{
		...
		{
			throw new ArgumentOutOfRangeException(nameof(separator),"UTF-16 surrogate pair is not allowed");
		}
	...
	public SplitResult Split(Utf8Span separator, Utf8StringSplitOptions options = Utf8StringSplitOptions.None)
	{
		...
		{
			throw new ArgumentException("separator cannotbe empty", nameof(separator));
		}
	...
	public readonly ref struct SplitResult
	{
		...
		{
			_state.DeconstructHelper(in _state.RemainingSearchSpace, out item1, out var remainder);
		...
		{
			private const Utf8StringSplitOptions HaltEnumeration = (Utf8StringSplitOptions)int.MinValue;
			...
			public bool MoveNext()
			{
				...
				if ((_state.SplitOptions & HaltEnumeration) != 0)
				{
					return false;
				}

				_state.SplitOptions |= HaltEnumeration; // prevents yielding <empty> forever at end of split

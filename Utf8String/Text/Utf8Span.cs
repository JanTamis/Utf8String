...
public readonly ref partial struct Utf8Span
{
	...
	/// </summary>
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	internal Utf8Span(ReadOnlySpan<byte> rawData)
	{
	...
	public Utf8Span this[Range range]
	{
		get
		{
			...
			ref var newRef = ref DangerousGetMutableReference(offset);
			...
			return UnsafeCreateWithoutValidation(MemoryMarshal.CreateReadOnlySpan(ref newRef, length));
		}
	}
	...
	{
		// Utf8Utility.GetIndexOfFirstNonWhiteSpaceChar(Bytes) == Length;
	...
	{
		throw new NotSupportedException("Equals(object) on Utf8Span will always throw an exception. use the Equals(Utf8Span) or operator == instead.");
	}
	...
	internal unsafe string ToStringNoReplacement()
	{
		...
		fixed (byte* pData = this)
		{
			...
			var utf16CharCount = Length + utf16CodeUnitCountAdjustment;
			...
			{
				var status = Utf8.ToUtf16(new ReadOnlySpan<byte>((byte*)state.pbData, state.cbData), chars, out _, out _, replaceInvalidSequences: false);

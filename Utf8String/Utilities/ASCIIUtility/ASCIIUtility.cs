...
internal static partial class ASCIIUtility
{
	...
	private static unsafe nuint GetIndexOfFirstNonAsciiByte_Intrinsified(byte* pBuffer, nuint bufferLength)
	{
		...
		var MaskOfAllBitsInVector128 = (nuint)(SizeOfVector128 - 1);
		...
		var bitmask = BitConverter.IsLittleEndian ? Vector128.Create((ushort)0x1001).AsByte() : Vector128.Create((ushort)0x0110).AsByte();
		...
		if (bufferLength >= 2 * SizeOfVector128)
		{
			var pFinalVectorReadPos = (byte*)((nuint)pBuffer + bufferLength - 2 * SizeOfVector128);
			...
			do
			{
				if (Sse2.IsSupported)
				{
					var firstVector = Sse2.LoadAlignedVector128(pBuffer);
					var secondVector = Sse2.LoadAlignedVector128(pBuffer + SizeOfVector128);
				...
				{
					var firstVector = AdvSimd.LoadVector128(pBuffer);
					var secondVector = AdvSimd.LoadVector128(pBuffer + SizeOfVector128);
		...
		if ((bufferLength & 8) != 0)
		{
			if (UIntPtr.Size == sizeof(ulong))
			{
				...
				var candidateUInt64 = Unsafe.ReadUnaligned<ulong>(pBuffer);
			...
			else
			{
				...
				var nextDWord = Unsafe.ReadUnaligned<uint>(pBuffer + 4);
	...
	private static unsafe nuint GetIndexOfFirstNonAsciiChar_Default(char* pBuffer, nuint bufferLength /* in chars */)
	{
		...
		{
			var SizeOfVectorInChars = (uint)Vector<ushort>.Count; // JIT will make this a const
			var SizeOfVectorInBytes = (uint)Vector<byte>.Count; // JIT will make this a const

			var maxAscii = new Vector<ushort>(0x007F);
			...
			if (Vector.LessThanOrEqualAll(Unsafe.ReadUnaligned<Vector<ushort>>(pBuffer), maxAscii))
			{
				...
				var pFinalVectorReadPos = pBuffer + bufferLength - SizeOfVectorInChars;
				pBuffer = (char*)(((nuint)pBuffer + SizeOfVectorInBytes) & ~(nuint)(SizeOfVectorInBytes - 1));

#if DEBUG
					var numCharsRead = pBuffer - pOriginalBuffer;
		...
		{
			currentUInt32 = Unsafe.ReadUnaligned<uint>(pBuffer);
			var nextUInt32 = Unsafe.ReadUnaligned<uint>(pBuffer + 4 / sizeof(char));
	...
	private static unsafe nuint GetIndexOfFirstNonAsciiChar_Sse2(char* pBuffer, nuint bufferLength /* in chars */)
	{
		...
		var SizeOfVector128InBytes = (uint)Unsafe.SizeOf<Vector128<byte>>();
		var SizeOfVector128InChars = SizeOfVector128InBytes / sizeof(char);
		...
		var asciiMaskForTestZ = Vector128.Create((ushort)0xFF80); // used for PTEST on supported hardware
		var asciiMaskForAddSaturate = Vector128.Create((ushort)0x7F80); // used for PADDUSW
		...
		if (bufferLength >= 2 * SizeOfVector128InBytes)
		{
			var pFinalVectorReadPos = (char*)((nuint)pBuffer + bufferLength - 2 * SizeOfVector128InBytes);
			...
			do
			{
				...
				var combinedVector = Sse2.Or(firstVector, secondVector);
		...
		if ((bufferLength & 4) != 0)
		{
			...
			else
			{
				...
				currentDWord = Unsafe.ReadUnaligned<uint>(pBuffer);
				var nextDWord = Unsafe.ReadUnaligned<uint>(pBuffer + 4 / sizeof(char));
	...
	public static unsafe nuint NarrowUtf16ToAscii(char* pUtf16Buffer, byte* pAsciiBuffer, nuint elementCount)
	{
		...
		{
			var SizeOfVector = (uint)Unsafe.SizeOf<Vector<byte>>(); // JIT will make this a const
			...
			if (elementCount >= 2 * SizeOfVector)
			{
				...
				var finalOffsetWhereCanLoop = elementCount - 2 * SizeOfVector;
				do
				{
					var utf16VectorHigh = Unsafe.ReadUnaligned<Vector<ushort>>(pUtf16Buffer + currentOffset);
					var utf16VectorLow = Unsafe.ReadUnaligned<Vector<ushort>>(pUtf16Buffer + currentOffset + Vector<ushort>.Count);
					...
					var asciiVector = Vector.Narrow(utf16VectorHigh, utf16VectorLow);
		...
		var remainingElementCount = elementCount - currentOffset;
		...
		{
			var finalOffsetWhereCanLoop = currentOffset + remainingElementCount - 4;
			do
			{
				if (IntPtr.Size >= 8)
				{
	...
	private static unsafe nuint NarrowUtf16ToAscii_Sse2(char* pUtf16Buffer, byte* pAsciiBuffer, nuint elementCount)
	{
		...
		var asciiMaskForTestZ = Vector128.Create(unchecked((short)0xFF80)); // used for PTEST on supported hardware
		var asciiMaskForAddSaturate = Vector128.Create((ushort)0x7F80); // used for PADDUSW
		...
		var utf16VectorFirst = Sse2.LoadVector128((short*)pUtf16Buffer); // unaligned load
		...
		var asciiVector = Sse2.PackUnsignedSaturate(utf16VectorFirst, utf16VectorFirst);
		...
		do
		{
			...
			utf16VectorFirst = Sse2.LoadVector128((short*)pUtf16Buffer + currentOffsetInElements); // unaligned load
			var utf16VectorSecond = Sse2.LoadVector128((short*)pUtf16Buffer + currentOffsetInElements + SizeOfVector128 / sizeof(short)); // unaligned load
			var combinedVector = Sse2.Or(utf16VectorFirst, utf16VectorSecond);
	...
	public static unsafe nuint WidenAsciiToUtf16(byte* pAsciiBuffer, char* pUtf16Buffer, nuint elementCount)
	{
		...
		else if (Vector.IsHardwareAccelerated)
		{
			...
			if (elementCount >= SizeOfVector)
			{
				...
				var finalOffsetWhereCanLoop = elementCount - SizeOfVector;
				do
				{
					var asciiVector = Unsafe.ReadUnaligned<Vector<sbyte>>(pAsciiBuffer + currentOffset);
					if (Vector.LessThanAny(asciiVector, Vector<sbyte>.Zero))
					{
						break; // found non-ASCII data
					}

					Vector.Widen(Vector.AsVectorByte(asciiVector), out var utf16LowVector, out var utf16HighVector);
	...
	private static unsafe nuint WidenAsciiToUtf16_Intrinsified(byte* pAsciiBuffer, char* pUtf16Buffer, nuint elementCount)
	{
		...
		if (!containsNonAsciiBytes)
		{
			...
			{
				var lower = AdvSimd.ZeroExtendWideningLower(asciiVector.GetLower());

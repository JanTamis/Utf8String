...
internal static partial class Utf8Utility
{
	...
	private static uint ExtractCharFromFirstTwoByteSequence(uint value)
	{
		...
		if (BitConverter.IsLittleEndian)
		{
			...
			var leadingByte = (uint)(byte)value << 6;
	...
	private static uint ExtractCharsFromFourByteSequence(uint value)
	{
		if (BitConverter.IsLittleEndian)
		{
			...
			// where wwww = uuuuu - 1
			var retVal = (uint)(byte)value << 8; // retVal = [ 00000000 00000000 11110uuu 00000000 ]
		...
		else
		{
			...
			// where wwww = uuuuu - 1
			var retVal = value & 0xFF00_0000u; // retVal = [ 11110uuu 00000000 00000000 00000000 ]
	...
	private static uint ExtractFourUtf8BytesFromSurrogatePair(uint value)
	{
		...
		if (BitConverter.IsLittleEndian)
		{
			...
			var tempA = BinaryPrimitives.ReverseEndianness(value & 0x003F_0700u); // = [ 00000000 00000uuu 00xxxxxx 00000000 ]
			tempA = BitOperations.RotateLeft(tempA, 16); // = [ 00xxxxxx 00000000 00000000 00000uuu ]

			var tempB = (value & 0x00FCu) << 6; // = [ 00000000 00000000 00uuzzzz 00000000 ]
			var tempC = (value >> 6) & 0x000F_0000u; // = [ 00000000 0000yyyy 00000000 00000000 ]
			tempC |= tempB;

			var tempD = (value & 0x03u) << 20; // = [ 00000000 00yy0000 00000000 00000000 ]
			tempD |= 0x8080_80F0u;
		...
		else
		{
			...
			var tempA = value & 0x0700_0000u; // = [ 00000uuu 00000000 00000000 00000000 ]
			var tempB = (value >> 2) & 0x003F_0000u; // = [ 00000000 00uuzzzz 00000000 00000000 ]
			tempB |= tempA;

			var tempC = (value << 2) & 0x0000_0F00u; // = [ 00000000 00000000 0000yyyy 00000000 ]
			var tempD = (value >> 4) & 0x0000_3000u; // = [ 00000000 00000000 00yy0000 00000000 ]
			tempD |= tempC;

			var tempE = (value & 0x3Fu) + 0xF080_8080u; // = [ 11110000 10000000 10000000 10xxxxxx ]
	...
	private static uint ExtractUtf8TwoByteSequenceFromFirstUtf16Char(uint value)
	{
		...
		if (BitConverter.IsLittleEndian)
		{
			...
			var temp = (value << 2) & 0x1F00u; // [ 00000000 00000000 000yyyyy 00000000 ]
		...
		else
		{
			...
			var temp = (value >> 16) & 0x3Fu; // [ 00000000 00000000 00000000 00xxxxxx ]
	...
	private static void WriteTwoUtf16CharsAsTwoUtf8ThreeByteSequences(ref byte outputBuffer, uint value)
	{
		...
		if (BitConverter.IsLittleEndian)
		{
			...
			var tempA = ((value << 2) & 0x3F00u) | ((value & 0x3Fu) << 16); // = [ 00000000 00xxxxxx 00yyyyyy 00000000 ]
			var tempB = ((value >> 4) & 0x0F00_0000u) | ((value >> 12) & 0x0Fu); // = [ 0000ZZZZ 00000000 00000000 0000zzzz ]
	...
	private static void WriteFirstUtf16CharAsUtf8ThreeByteSequence(ref byte outputBuffer, uint value)
	{
		...
		if (BitConverter.IsLittleEndian)
		{
			...
			var tempA = (value << 2) & 0x3F00u; // [ 00yyyyyy 00000000 ]
			var tempB = (uint)(ushort)value >> 12; // [ 00000000 0000zzzz ]

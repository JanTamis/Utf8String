...
internal static unsafe partial class Utf8Utility
{
	...
	public static OperationStatus TranscodeToUtf16(byte* pInputBuffer, int inputLength, char* pOutputBuffer, int outputCharsRemaining, out byte* pInputBufferRemaining, out char* pOutputBufferRemaining)
	{
		...
		{
			var numElementsConverted = ASCIIUtility.WidenAsciiToUtf16(pInputBuffer, pOutputBuffer, (uint)Math.Min(inputLength, outputCharsRemaining));
		...
		var pFinalPosWhereCanReadDWordFromInputBuffer = pInputBuffer + (uint)inputLength - 4;
		...
		do
		{
			...
			var thisDWord = Unsafe.ReadUnaligned<uint>(pInputBuffer);
			...
			if (ASCIIUtility.AllBytesInUInt32AreAscii(thisDWord))
			{
				...
				var remainingInputBytes = (uint)(void*)Unsafe.ByteOffset(ref *pInputBuffer, ref *pFinalPosWhereCanReadDWordFromInputBuffer) + 4;
				var maxIters = Math.Min(remainingInputBytes, (uint)outputCharsRemaining) / (2 * sizeof(uint));
				uint secondDWord;
				int i;
			...
			if (UInt32FirstByteIsAscii(thisDWord))
			{
				if (outputCharsRemaining >= 3)
				{
					...
					var thisDWordLittleEndian = ToLittleEndian(thisDWord);
			...
			if (UInt32BeginsWithUtf8TwoByteMask(thisDWord))
			{
				...
				var charToWrite = ExtractCharFromFirstTwoByteSequence(thisDWord); // optimistically compute this now, but don't store until we know dest is large enough
				...
				if (BitConverter.IsLittleEndian)
				{
					...
					if (((thisDWord - 0xE000_0000u) & 0xF000_0000u) == 0)
					{
						...
						if (outputCharsRemaining > 1 && (nint)(void*)Unsafe.ByteOffset(ref *pInputBuffer, ref *pFinalPosWhereCanReadDWordFromInputBuffer) >= 3)
						{
							...
							var secondDWord = Unsafe.ReadUnaligned<uint>(pInputBuffer + 3);
...

			{
				...
				if (BitConverter.IsLittleEndian)
				{
					...
					var toCheck = thisDWord & 0x0000_FFFFu;
		...
		while (inputLength > 0)
		{
			...
			if ((byte)firstByte <= 0xDFu - 0xC2u)
			{
				...
				var asChar = (firstByte << 6) + secondByte + ((0xC2u - 0xC0u) << 6) - 0x80u; // remove UTF-8 markers from scalar
			...
			else if ((byte)firstByte <= 0xEFu - 0xC2u)
			{
				...
				if (inputLength >= 3)
				{
					...
					var partialChar = (firstByte << 12) + (secondByte << 6);
				...
				else if (inputLength >= 2)
				{
					...
					var partialChar = (firstByte << 6) + secondByte; // don't worry about fixing up the UTF-8 markers; we'll account for it in the below comparison
			...
			else if ((byte)firstByte <= 0xF4u - 0xC2u)
			{
				...
				var asPartialChar = (firstByte << 6) + nextByte; // don't worry about fixing up the UTF-8 markers; we'll account for it in the below comparison
		...
		var retVal = OperationStatus.Done;
		goto ReturnCommon;
	...
	public static OperationStatus TranscodeToUtf8(char* pInputBuffer, int inputLength, byte* pOutputBuffer, int outputBytesRemaining, out char* pInputBufferRemaining, out byte* pOutputBufferRemaining)
	{
		...
		{
			var numElementsConverted = ASCIIUtility.NarrowUtf16ToAscii(pInputBuffer, pOutputBuffer, (uint)Math.Min(inputLength, outputBytesRemaining));
		...
		var pFinalPosWhereCanReadDWordFromInputBuffer = pInputBuffer + (uint)inputLength - CharsPerDWord;
		...
		do
		{
			...
			if (Utf16Utility.AllCharsInUInt32AreAscii(thisDWord))
			{
				...
				var valueToWrite = thisDWord | (thisDWord >> 8);
				...
				var inputCharsRemaining = (uint)(pFinalPosWhereCanReadDWordFromInputBuffer - pInputBuffer) + 2;
				var minElementsRemaining = (uint)Math.Min(inputCharsRemaining, outputBytesRemaining);

				if (Sse41.X64.IsSupported || (AdvSimd.Arm64.IsSupported && BitConverter.IsLittleEndian))
				{
					// Try reading and writing 8 elements per iteration.
					var maxIters = minElementsRemaining / 8;
					...
					for (i = 0; (uint)i < maxIters; i++)
					{
						...
						{
							var isUtf16DataNonAscii = AdvSimd.CompareTest(utf16Data, nonAsciiUtf16DataMask);
							var hasNonAsciiDataInVector = AdvSimd.Arm64.MinPairwise(isUtf16DataNonAscii, isUtf16DataNonAscii).AsUInt64().ToScalar() != 0;
							...
							var lower = AdvSimd.ExtractNarrowingSaturateUnsignedLower(utf16Data);
				...
				else
				{
					...
					var maxIters = minElementsRemaining / 4;
					uint secondDWord;
					int i;
					for (i = 0; (uint)i < maxIters; i++)
					{

...
internal static unsafe partial class Utf8Utility
{
	...
	public static byte* GetPointerToFirstInvalidByte(byte* pInputBuffer, int inputLength, out int utf16CodeUnitCountAdjustment, out int scalarCountAdjustment)
	{
		...
		{
			var numAsciiBytesCounted = ASCIIUtility.GetIndexOfFirstNonAsciiByte(pInputBuffer, (uint)inputLength);
			...
			var pOriginalInputBuffer = pInputBuffer;
			var originalInputLength = inputLength;
#endif

		// Enregistered locals that we'll eventually out to our caller.

		var tempUtf16CodeUnitCountAdjustment = 0;
		...
		var pFinalPosWhereCanReadDWordFromInputBuffer = pInputBuffer + (uint)inputLength - sizeof(uint);
		...
		while (pInputBuffer <= pFinalPosWhereCanReadDWordFromInputBuffer)
		{
			...
			if (ASCIIUtility.AllBytesInUInt32AreAscii(thisDWord))
			{
				...
				if ((nint)(void*)Unsafe.ByteOffset(ref *pInputBuffer, ref *pFinalPosWhereCanReadDWordFromInputBuffer) >= 4 * sizeof(uint))
				{
					...
					var pInputBufferFinalPosAtWhichCanSafelyLoop = pFinalPosWhereCanReadDWordFromInputBuffer - 3 * sizeof(uint); // can safely read 4 DWORDs here
					...
					if (AdvSimd.Arm64.IsSupported && BitConverter.IsLittleEndian)
					{
						...
						var bitMask128 = BitConverter.IsLittleEndian ? Vector128.Create((ushort)0x1001).AsByte() : Vector128.Create((ushort)0x0110).AsByte();
						do
						{
							var mask = GetNonAsciiBytes(AdvSimd.LoadVector128(pInputBuffer), bitMask128);
							if (mask != 0)
							{
					...
					else
					{
						do
						{
							if (Sse2.IsSupported)
							{
								var mask = (uint)Sse2.MoveMask(Sse2.LoadVector128(pInputBuffer));
								if (mask != 0)
								{
			...
			{
				var numLeadingAsciiBytes = ASCIIUtility.CountNumberOfLeadingAsciiBytesFromUInt32WithSomeNonAsciiData(thisDWord);
			...
			if ((thisDWord & (BitConverter.IsLittleEndian ? 0x00C0_C0F0u : 0xF0C0_C000u)) == 0)
			{
				...
				if (IntPtr.Size >= 8 && BitConverter.IsLittleEndian)
				{
					...
					{
						var thisQWord = Unsafe.ReadUnaligned<ulong>(pInputBuffer);

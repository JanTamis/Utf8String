...
internal static unsafe partial class Utf16Utility
{
	...
	public static char* GetPointerToFirstInvalidChar(char* pInputBuffer, int inputLength, out long utf8CodeUnitCountAdjustment, out int scalarCountAdjustment)
	{
		...
		var numAsciiCharsConsumedJustNow = (int)ASCIIUtility.GetIndexOfFirstNonAsciiChar(pInputBuffer, (uint)inputLength);
		...
		var tempScalarCountAdjustment = 0;
		var pEndOfInputBuffer = pInputBuffer + (uint)inputLength;
		...
		if ( /* (AdvSimd.Arm64.IsSupported && BitConverter.IsLittleEndian) || */ Sse41.IsSupported)
		{
			...
			{
				var vector0080 = Vector128.Create((ushort)0x0080);
				var vector7800 = Vector128.Create((ushort)0x7800);
				var vectorA000 = Vector128.Create((ushort)0xA000);

				var bitMask128 = BitConverter.IsLittleEndian ? Vector128.Create(0x80402010_08040201).AsByte() : Vector128.Create(0x01020408_10204080).AsByte();

				var pHighestAddressWhereCanReadOneVector = pEndOfInputBuffer - Vector128<ushort>.Count;
				...
				do
				{
					...
					else
					{
						...
						var lowSurrogatesMask = mask2 & mask;
						...
						var highSurrogatesMask = (mask2 ^ 0b_0101_0101_0101_0101u /* flip all even-numbered bits 00 <-> 01 */) & mask;
		...
		else if (Vector.IsHardwareAccelerated)
		{
			...
			{
				var vector0080 = new Vector<ushort>(0x0080);
				var vector0400 = new Vector<ushort>(0x0400);
				var vector0800 = new Vector<ushort>(0x0800);
				var vectorD800 = new Vector<ushort>(0xD800);

				var pHighestAddressWhereCanReadOneVector = pEndOfInputBuffer - Vector<ushort>.Count;
				Debug.Assert(pHighestAddressWhereCanReadOneVector >= pInputBuffer);
				...
				do
				{
					...
					var utf16Data = Unsafe.ReadUnaligned<Vector<ushort>>(pInputBuffer);
					var twoOrMoreUtf8Bytes = Vector.GreaterThanOrEqual(utf16Data, vector0080);
					var threeOrMoreUtf8Bytes = Vector.GreaterThanOrEqual(utf16Data, vector0800);
					var sumVector = (Vector<nuint>)(Vector<ushort>.Zero - twoOrMoreUtf8Bytes - threeOrMoreUtf8Bytes);
					...
					nuint popcnt = 0;
					for (var i = 0; i < Vector<nuint>.Count; i++)
					{
						popcnt += (nuint)sumVector[i];
					}

					var popcnt32 = (uint)popcnt;
					if (IntPtr.Size == 8)
					{
					...
					var surrogateChars = Vector.LessThan(utf16Data, vector0800);
					if (surrogateChars != Vector<ushort>.Zero)
					{
						...
						var highSurrogateChars = Vector.LessThan(utf16Data, vector0400);
						var lowSurrogateChars = Vector.AndNot(surrogateChars, highSurrogateChars);
						...
						for (var i = 0; i < Vector<ushort>.Count - 1; i++)
						{
						...
						var surrogatePairsCountNint = (nint)surrogatePairsCount; // zero-extend to native int size
	...
	private static uint GetNonAsciiBytes(Vector128<byte> value, Vector128<byte> bitMask128)
	{
		...
		var extractedBits = AdvSimd.And(mostSignificantBitIsSet, bitMask128);

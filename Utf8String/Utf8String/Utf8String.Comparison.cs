...
public sealed partial class Utf8String
{
	...
	public static bool AreEquivalent(Utf8String? utf8Text, string? utf16Text)
	{
		...
		if (utf8Text.Length < utf16Text.Length || (uint)utf16Text.Length * MAX_UTF8_BYTES_PER_UTF16_CHAR < (uint)utf8Text.Length)
		{
			return false;
		}
	...
	private static bool AreEquivalentOrdinalSkipShortCircuitingChecks(ReadOnlySpan<byte> utf8Text, ReadOnlySpan<char> utf16Text)
	{
		while (!utf16Text.IsEmpty)
		{
			...
			if (Rune.DecodeFromUtf16(utf16Text, out var scalarFromUtf16, out var charsConsumedJustNow) != OperationStatus.Done
			    || Rune.DecodeFromUtf8(utf8Text, out var scalarFromUtf8, out var bytesConsumedJustNow) != OperationStatus.Done
	...
	{
		return Rune.TryCreate(value, out var rune) && Contains(rune);
	}
	...
	{
		return Rune.TryCreate(value, out var rune) && Contains(rune, comparison);
	}
	...
	public bool Contains(Rune value)
	{
		...
		var runeBytesWritten = value.EncodeToUtf8(runeBytes);
	...
	{
		return Rune.TryCreate(value, out var rune) && EndsWith(rune);
	}
	...
	{
		return Rune.TryCreate(value, out var rune) && EndsWith(rune, comparison);
	}
	...
	{
		return Rune.TryCreate(value, out var rune) && StartsWith(rune);
	}
	...
	{
		return Rune.TryCreate(value, out var rune) && StartsWith(rune, comparison);
	}

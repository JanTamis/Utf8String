...
public sealed partial class Utf8String
{
	...
	{
		throw new ArgumentOutOfRangeException(nameof(options), options, "Invalid Utf8StringSplitOptions value.");
	}
	...
	private Utf8String InternalSubstring(int startIndex, int length)
	{
		...
		Debug.Assert(startIndex <= Length, "StartIndex cannot point beyond the end of the string (except to the null terminator).");
		Debug.Assert(length >= 0, "Length cannot be negative.");
		Debug.Assert(startIndex + length <= Length, "StartIndex and Length cannot point beyond the end of the string.");

		Debug.Assert(length != 0 && length != Length, "Caller should handle Length boundary conditions.");
		...
		var newString = FastAllocateSkipZeroInit(length);
		Unsafe.CopyBlockUnaligned(ref MemoryMarshal.GetArrayDataReference(newString), ref DangerousGetMutableReference(startIndex), (uint)length);
	...
	{
		throw new InvalidOperationException("Cannot split a Utf8String instance into two parts that would split a multi-byte UTF-8 subsequence.");
	}
	...
	{
		if (!Rune.TryCreate(separator, out var rune))
		{
			throw new ArgumentOutOfRangeException(nameof(separator), "The specified separator is not a valid Unicode code point.");
		}
	...
	public SplitResult Split(Utf8String separator, Utf8StringSplitOptions options = Utf8StringSplitOptions.None)
	{
		...
		{
			throw new ArgumentException("The specified separator cannot be null or empty.", nameof(separator));
		}
	...
	{
		return TryFind(separator, out var range) ? new SplitOnResult(this, range) : new SplitOnResult(this);
	}
	...
	{
		return TryFind(separator, comparisonType, out var range) ? new SplitOnResult(this, range) : new SplitOnResult(this);
	}
	...
	{
		return TryFindLast(separator, out var range) ? new SplitOnResult(this, range) : new SplitOnResult(this);
	}
	...
	{
		return TryFindLast(separator, comparisonType, out var range) ? new SplitOnResult(this, range) : new SplitOnResult(this);
	}
	...
	{
		var trimmedSpan = this.AsSpan().TrimHelper(trimType);
		...
		return trimmedSpan.Length == Length ? this : trimmedSpan.ToUtf8String();
	}
	...
	public readonly struct SplitResult : IEnumerable<Utf8String?>
	{
		...
		{
			_state.DeconstructHelper(_state.GetRemainingSearchSpace(), out var nextItem, out var remainder);
		...
		public struct Enumerator : IEnumerator<Utf8String?>
		{
			...
			{
				var wasMatchFound = _state.DeconstructHelper(_state.GetRemainingSearchSpace(), out var firstItem, out var remainder);
		...
		private struct State // fully mutable
		{
			...
			internal readonly bool DeconstructHelper(in Utf8Span source, out Utf8Span firstItem, out Utf8Span remainder)
			{
				...
				bool wasMatchFound;
				ref readonly var searchSpan = ref source;
				...
				while (true)
				{
					...
					var searchRune = SearchRune; // local copy so as to avoid struct tearing
					if (searchRune >= 0)
					{
	...
	public readonly struct SplitOnResult
	{
		...
		{
			var (startIndex, length) = searchTermMatchRange.GetOffsetAndLength(originalSearchSpace.Length);
			...
			Before = originalSearchSpace[..startIndex].ToUtf8String();
			After = originalSearchSpace[(startIndex + length)..].ToUtf8String();
		}

...
public static class Utf8Extensions
{
	...
	public static ReadOnlySpan<byte> AsBytes(this Utf8String? text)
	{
		...
		return MemoryMarshal.CreateSpan(ref text.DangerousGetMutableReference(), text.Length);
	}
	...
	public static ReadOnlySpan<byte> AsBytes(this Utf8String? text, int start)
	{
		...
		{
			if (start != 0)
				throw new ArgumentOutOfRangeException(nameof(start), start, "The start index must be 0.");
			return default;
		}

		if ((uint)start > (uint)text.Length)
			throw new ArgumentOutOfRangeException(nameof(start), start, "The start index cannot be greater than the length of the string.");

		return MemoryMarshal.CreateReadOnlySpan(ref text.DangerousGetMutableReference(start), text.Length - start);
	}
	...
	public static ReadOnlySpan<byte> AsBytes(this Utf8String? text, int start, int length)
	{
		...
		if ((uint)start > (uint)text.Length || (uint)length > (uint)(text.Length - start))
			throw new ArgumentOutOfRangeException(nameof(start), start, "The start index or length is out of range.");
#endif

		return MemoryMarshal.CreateReadOnlySpan(ref text.DangerousGetMutableReference(start), length);
	}
	...
	public static Utf8Span AsSpan(this Utf8String? text, int start)
	{
		...
		return Utf8Span.UnsafeCreateWithoutValidation(MemoryMarshal.CreateReadOnlySpan(ref text.DangerousGetMutableReference(start), text.Length - start));
	}
	...
	public static Utf8Span AsSpan(this Utf8String? text, int start, int length)
	{
		...
		return Utf8Span.UnsafeCreateWithoutValidation(MemoryMarshal.CreateReadOnlySpan(ref text.DangerousGetMutableReference(start), length));
	}
...
// 				throw new ArgumentOutOfRangeException(nameof(startIndex), actualIndex, "The start index cannot be greater than the length of the string.");
//
...
//
// 			if ((uint)start > (uint)text.Length)
// 				throw new ArgumentOutOfRangeException(nameof(start), start, "The start index is out of range.");
//
...
// 			var actualIndex = startIndex.GetOffset(text.Length);
// 			if ((uint)actualIndex > (uint)text.Length)
// 				throw new ArgumentOutOfRangeException(nameof(startIndex), actualIndex, "The start index is out of range.");
//
